/**
 * Admin.jsx  â€“  full admin panel with 3 tabs:
 *   1. Dashboard (stats + elections + party vote counts)
 *   2. Create Election (form â†’ blockchain + MongoDB)
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      <div className="mb-8 flex items-center gap-3">
        <div className="w-10 h-10 bg-purple-600 rounded-xl flex items-center justify-center text-2xl">âš™</div>
        <div>
...
function TabDashboard({ API }) {
  ...
  if (loading) return <div className="text-center text-slate-400 py-16">Loadingâ€¦</div>;
  if (!data) return null;
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-slate-400 text-sm">
                    {election.state} Â· onChainId: #{election.onChainId ?? "â€”"} Â· {election.totalVotes} votes
                  </p>
                  <p className="text-slate-500 text-xs">
                    {new Date(election.startTime).toLocaleDateString()} â€“ {new Date(election.endTime).toLocaleDateString()}
                  </p>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  >
                    {syncing[election._id] ? "Syncingâ€¦" : "Sync Votes"}
                  </button>
                </div>
              </div>
                      ...
                      <div key={party._id} className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between text-xs text-slate-300 mb-1">
                            <span>{party.partyName} â€” {party.candidateName}</span>
            ...
            {data.recentUsers.map((u) => (
              <div key={u._id} className="p-4 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-xs">{u.email} Â· {new Date(u.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="max-w-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-blue-600/10 border border-blue-500/30 rounded-lg p-3 text-sm text-blue-300">
          âš  Creating will deploy the election to the Sepolia blockchain immediately. This may take 15â€“30 seconds.
        </div>
...
function TabAddParty({ API }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Party "${form.partyName}" added! TX: ${data.txHash?.slice(0, 10)}â€¦`);
    ...
    <div className="max-w-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-yellow-600/10 border border-yellow-500/30 rounded-lg p-3 text-sm text-yellow-300">
          âš  Adding a party will call <code>addCandidate</code> on the blockchain. This requires gas and takes ~15 seconds.
        </div>
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-60">
          ...
          />
          {search && <button onClick={() => handleSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white">✕</button>}
        </div>
      ...
      {(gender || status) && (
        <div className="flex gap-2 flex-wrap">
          {gender && <span className="bg-blue-600/20 border border-blue-500/30 text-blue-300 text-xs px-3 py-1 rounded-full flex items-center gap-1">Gender: {gender} <button onClick={() => handleFilter("", status)} className="ml-1 hover:text-white">✕</button></span>}
          {status && <span className="bg-purple-600/20 border border-purple-500/30 text-purple-300 text-xs px-3 py-1 rounded-full flex items-center gap-1">Status: {status} <button onClick={() => handleFilter(gender, "")} className="ml-1 hover:text-white">✕</button></span>}
